...
public class ShootSpreadGlobalProjectile : GlobalProjectile
{
    public override void OnSpawn(Projectile projectile, IEntitySource source)
    {
        ...
        if (player.whoAmI == Main.myPlayer && projectile.DamageType == DamageClass.Ranged && source is EntitySource_ItemUse_WithAmmo itemSource)
        {
            float spreadReduction = player.GetModPlayer<MacrocosmPlayer>().ShootSpreadReduction;
            if (spreadReduction > 0f && !ItemSets.IgnoresShootSpreadReduction[itemSource.Item.type])
            {
