...
namespace Macrocosm.Common.TileFrame
{
    public class TileAnimation : ILoadable
    {
        ...
        private readonly struct AnimationData(int frameMax, int frameCounterMax, int[] frameData, int index = int.MinValue)
        {
            ...
            public override bool Equals(object obj)
            {
                return obj is AnimationData data &&
                       ...
                       EqualityComparer<int[]>.Default.Equals(FrameData, data.FrameData);
            }
        }

        public static void NewTemporaryAnimation(int type, ushort tileType, int x, int y) => Animation.NewTemporaryAnimation(type, tileType, x, y);
        ...
        private void On_Animation_SetDefaults(On_Animation.orig_SetDefaults orig, Animation self, int type)
        {
            ...
            {
                typeof(Animation).SetFieldValue("_frameMax", customAnimations[type].FrameMax);
                typeof(Animation).SetFieldValue("_frameCounterMax", customAnimations[type].FrameCounterMax);
                typeof(Animation).SetFieldValue("_frameData", customAnimations[type].FrameData);
            }
        }
