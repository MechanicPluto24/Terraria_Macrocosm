...
namespace Macrocosm.Common.Drawing
{
	...
	public static class CelestialBody1
	{
		/// <summary>
		...
		public static void Draw(Texture2D bodyTexture, Texture2D atmoTexture, float scale, float averageOffsetX = 0f, float averageOffsetY = 0f, float parallax_X = 0f, float parallax_Y = 0f)
		{
			...
			//Vector2 earthstaticpos = new Vector2(Main.screenWidth / 2 - averageOffsetX, Main.screenHeight / 2 - averageOffsetY);
			Vector2 sunpos = RotatingCelestialBody.pos;

			float rot = (position - sunpos).ToRotation();// * MathHelper.Pi;// * (Main.screenWidth / 2 / Vector2.Distance(Main.MouseScreen, earthstaticpos));


			double abstime = Main.dayTime ? Main.time : Main.time + Main.dayLength;

			Color color = Color.White;
			
			color.A = (byte)(255f - 255f * ((abstime-27000)/(43200)));

			if(abstime <= 27000)
				color.A = (byte)(102f + 153f * ((abstime) / (27000)));

			if(abstime >= 70200)
				color.A = (byte)(102f * ((abstime-70200) / (16200)));
			...
			if(Main.time % 20 == 0)
				Main.NewText(abstime.ToString() + ", " + color.A);
			...
			Texture2D atmoshadow = Terraria.ModLoader.ModContent.Request<Texture2D>("Macrocosm/Content/Backgrounds/Moon/EarthAtmoShadowMask").Value;

			Texture2D shadow = Terraria.ModLoader.ModContent.Request<Texture2D>("Macrocosm/Content/Backgrounds/Moon/EarthShadowMask").Value;

			Main.spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Main.DefaultSamplerState, DepthStencilState.None, RasterizerState.CullCounterClockwise, null, Main.GameViewMatrix.EffectMatrix);
			Main.spriteBatch.Draw(atmoshadow, position, null, color, rot, atmoTexture.Size() / 2, scale, default, 0f);
			Main.spriteBatch.End();

			Main.spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Main.DefaultSamplerState, DepthStencilState.None, RasterizerState.CullCounterClockwise, null, Main.GameViewMatrix.EffectMatrix);
			Main.spriteBatch.Draw(shadow, position, null, color, rot, atmoTexture.Size() / 2, scale, default, 0f);
			Main.spriteBatch.End();
