...
public class TorchLightHooks : ILoadable
{
    ...
    {
        On_WorldItem.UpdateItem_VisualEffects += RemoveDroppedItemLight; // removes lighting for dropped items
    ...
    public void Unload()
    {
        On_WorldItem.UpdateItem_VisualEffects -= RemoveDroppedItemLight;
    ...
    private static void RemoveDroppedItemLight(On_WorldItem.orig_UpdateItem_VisualEffects orig, WorldItem item)
    {
    ...
    private static bool DisableTorchHolding(On_Player.orig_CanVisuallyHoldItem orig, Player self, Item item)
    {
        if (SubworldSystem.AnyActive<Macrocosm>()
            && ItemID.Sets.Torches[item.type]
            && !ItemSets.AllowedTorches[item.type]
            && !RoomOxygenSystem.CheckRoomOxygen(self.Center))
            return false;
