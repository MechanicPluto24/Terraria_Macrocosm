...
{
    /// <summary> Buff/Debuff Sets for special behavior of some buffs/debuffs, useful for crossmod.  </summary>
    ...
    public class BuffSets
    {
        ...
        public static int[] TypicalDuration { get; } = BuffID.Sets.Factory.CreateNamedSet(nameof(TypicalDuration)).Description("Typical duration of buff/debuff. Usually used when applied randomly from a pool").RegisterIntSet();

        /// <summary> The severity of a radiation-related debuff. </summary>
        public static RadiationSeverity[] RadiationBuffSeverity { get; } = BuffID.Sets.Factory.CreateNamedSet(nameof(RadiationBuffSeverity)).Description("The severity of a radiation-related debuff.").RegisterCustomSet(defaultState: RadiationSeverity.None);
