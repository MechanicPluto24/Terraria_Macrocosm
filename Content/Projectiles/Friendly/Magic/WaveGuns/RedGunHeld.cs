            ...
            if (Player.whoAmI == Main.myPlayer)
            {
                ...
                float knockback = currentItem.knockBack;
                    if (AI_Timer % currentItem.useTime == 0)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Normalize(Projectile.velocity.RotatedByRandom(MathHelper.PiOver2/9)) *25f, ModContent.ProjectileType<RedEnergyBolt>(), damage, knockback, Projectile.owner);
