...
namespace Macrocosm.Content.Projectiles.Friendly.Melee
{
    public class CrescentScriptureProjectile : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            // Back part
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, frameY: 0), backDarkColor * lightingColor * transparency, rotation2 + SwingDirection * MathHelper.PiOver4 * -1f * (1f - transparency), Origin, scale, spriteEffects, 0f);
            ...
            // Middle part
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, frameY: 0), middleMediumColor * lightingColor * transparency * 0.8f, rotation2, Origin, scale, spriteEffects, 0f);
            // Front part
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, frameY: 0), frontLightColor * lightingColor * transparency * 0.5f, rotation2, Origin, scale * 0.975f, spriteEffects, 0f);
            ...
            {
                float edgeRotation = rotation2 + SwingDirection * i * (MathHelper.Pi * -2f) * 0.025f + Utils.Remap(transparency, 0f, 1f, 0f, MathHelper.PiOver4) * SwingDirection;
                Vector2 drawPos = position + edgeRotation.ToRotationVector2() * ((float)texture.Width * 0.5f - 6f) * scale;
                Utility.DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawPos, new Color(255, 255, 255, 0) * lerpTime * (i / 9f), middleMediumColor, transparency, 0f, 0.5f, 0.5f, 1f, edgeRotation, new Vector2(0f, Utils.Remap(transparency, 0f, 1f, 3f, 0f)) * scale, Vector2.One * scale);
            }

            // This draws a large star sparkle at the front of the projectile.
            Vector2 drawPos2 = position + (rotation2 + Utils.Remap(transparency, 0f, 1f, 0f, MathHelper.PiOver4) * SwingDirection).ToRotationVector2() * ((float)texture.Width * 0.5f - 4f) * scale;
