...
{
    public class MegaBladeProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            if (!spawned)
            {
                Projectile.velocity = Projectile.Center.DirectionTo(Projectile.Center+new Vector2(10*SwingDirection,0));
                Projectile.netUpdate = true;
                spawned = true;
            }
            ...
            if (Player.noItems || Player.CCed || Player.ItemAnimationEndingOrEnded || Player.HeldItem.type != ModContent.ItemType<MegaBlade>())
            {
                despawn = true;
                return;
            }
            ...
            Projectile.rotation = Projectile.velocity.ToRotation() + (0.5f * Arc - Arc * (-MathF.Sin((progress * progress * progress - 0.5f) * MathHelper.Pi) + 1f) / 2f) * SwingDirection;
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color secondaryColor = (new Color(120, 120, 120)).WithOpacity(progressScale);
        ...
        {
            float _ = 0;
            Vector2 hitboxEnd = Projectile.Center + ((Projectile.rotation - MathHelper.PiOver4) * Player.direction + (Player.direction == -1 ? MathHelper.Pi : 0f)).ToRotationVector2() * (100f);
            return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), Projectile.Center, hitboxEnd, 100f, ref _);
        }
    }
}