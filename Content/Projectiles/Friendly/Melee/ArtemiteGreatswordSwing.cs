...
namespace Macrocosm.Content.Projectiles.Friendly.Melee
{
    public class ArtemiteGreatswordSwing : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Main.spriteBatch.Draw(val.Value, vector, frame, color * brightness * progressScale, Projectile.rotation + Projectile.ai[0] * ((float)Math.PI / 4f) * -1f * (1f - progress), origin, scale * 0.95f, effects, 0f);
 			...
 			for (float f = 0f; f < 16f; f += 0.5f)
			{
				...
				Vector2 drawpos = vector + angle.ToRotationVector2() * ((float)val.Width() * 0.5f - 8f) * scale;
