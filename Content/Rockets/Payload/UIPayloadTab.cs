...
namespace Macrocosm.Content.Rockets.Payload
{
	public class UIPayloadTab : UIPanel, ITabUIElement, IRocketDataConsumer
    {
		...
		{ 
			...
			set 
			{
				cacheSize = rocket.InventorySize;
				bool newSize = cacheSize != value.InventorySize;
				rocket = value;
		...
		public override void Update(GameTime gameTime)
        {
			if (Main.LocalPlayer.controlQuickHeal)
				rocket.InventorySize += 1;

			if (Main.LocalPlayer.controlQuickMana)
				rocket.InventorySize -= 1;

			if (cacheSize != rocket.InventorySize)
			{
				this.ReplaceChildWith(inventoryPanel, inventoryPanel = CreateInventory());
				cacheSize = rocket.InventorySize;
			}
		...
		private UIListScrollablePanel CreateInventory()
		{
			...
			int count = rocket.Inventory.Length;

			int iconsPerRow = 10;
			int rowsWithoutScrollbar = 13;
			float iconSize;
