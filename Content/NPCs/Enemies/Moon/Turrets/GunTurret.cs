...
namespace Macrocosm.Content.NPCs.Enemies.Moon.Turrets
{
    public class GunTurret : ModNPC
    {
        ...
        private Vector2 TurretHeight=new Vector2(0,-18);
        ...
        public override void AI()
        {
            ...
            if (NPC.HasPlayerTarget && clearLineOfSight)
            {
                ...
                if(AI_Timer==60)
                {
                    Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center+TurretHeight+new Vector2(30,0).RotatedBy(TurretRotation), new Vector2(25f,0).RotatedBy(TurretRotation), ModContent.ProjectileType<ZombieSecurityBullet>(), 100, 1f, Main.myPlayer);
                }
                if(AI_Timer==70)
                {
                    Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center+TurretHeight+new Vector2(30,0).RotatedBy(TurretRotation), new Vector2(25f,0).RotatedBy(TurretRotation), ModContent.ProjectileType<ZombieSecurityBullet>(), 100, 1f, Main.myPlayer);
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            spriteBatch.Draw(turretTexture.Value, NPC.Center + TurretHeight - Main.screenPosition + new Vector2(NPC.direction > 0 ? 5 : -5,0), null, drawColor, TurretRotation, turretTexture.Size() / 2, NPC.scale,  NPC.direction > 0 ? SpriteEffects.None : SpriteEffects.FlipVertically, 0f);
        }
